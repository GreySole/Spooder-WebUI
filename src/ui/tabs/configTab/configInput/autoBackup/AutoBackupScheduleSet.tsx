...
interface AutoBackupScheduleSetProps {
  formKey: string;
}

export default function AutoBackupScheduleSet({ formKey }: AutoBackupScheduleSetProps) {
  ...
  useEffect(() => {
    if (frequency == null || time == null || maxBackups == null) {
      ...
      setMaxBackups(getValues(`${formKey}.max_backups`));
      return;
    }
    const scheduleString = generateScheduleString(frequency, time);
    setValue(`${formKey}.schedule`, scheduleString);
    setValue(`${formKey}.max_backups`, maxBackups);
  }, [frequency, time, maxBackups, setValue, setMaxBackups, formKey]);
  return (
    <>
      <SelectDropdown
        label='Frequency'
      ...
      <NumberInput
        label='Max Backups'
        value={maxBackups ?? 0}
        onInput={(value) => setMaxBackups(value)}
      />
    </>
  );
}
