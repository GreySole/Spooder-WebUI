...
import FormBoolSwitch from '../Common/input/form/FormBoolSwitch.js';
...
class DiscordTab extends React.Component {
  ...
  handleDiscordChange = (s) => {
    ...
    if (name.includes('-')) {
      name = name.split('-');
      if (newDiscord.config[name[0]] == null) {
        newDiscord.config[name[0]] = {};
      }
  ...
  render() {
    ...
    if (this.state.guilds != null) {
      if (this.state.config.autosendngrok?.enabled) {
        for (let g in this.state.guilds) {
          guildOptions.push(<option value={g}>{this.state.guilds[g].name}</option>);
        }
      }

      if (this.state.config.autosendngrok?.enabled) {
        if (
          this.state.config.autosendngrok.destguild != null &&
          this.state.config.autosendngrok.destguild != ''
        ) {
          for (let c in this.state.guilds[this.state.config.autosendngrok.destguild].channels) {
            channelOptions.push(
              <option value={c}>
                {this.state.guilds[this.state.config.autosendngrok.destguild].channels[c].name}
              </option>,
            );
          }
        }
    ...
    let autoNgrokFields =
      this.state.config?.autosendngrok?.enabled != false ? (
        <div className='config-variable'>
          <label>
            Server
            <select
              name='autosendngrok-destguild'
              defaultValue={this.state.config.autosendngrok?.destguild}
              onChange={this.handleDiscordChange}
            >
              {guildOptions}
            </select>
          </label>
          <label>
            Channel
            <select
              name='autosendngrok-destchannel'
              defaultValue={this.state.config.autosendngrok?.destchannel}
              onChange={this.handleDiscordChange}
            >
              {channelOptions}
            </select>
        ...
        <div className='config-variable'>
          <label>
            ...
            <input
              name='master'
              type='text'
              defaultValue={this.state.config?.master}
              onChange={this.handleDiscordChange}
            />
          </label>
        </div>
        <div className='config-variable'>
          <label>
            ...
            <input
              name='token'
              type='password'
              defaultValue={this.state.config?.token}
              onChange={this.handleDiscordChange}
            />
          </label>
        </div>
    ...
    let configFields = (
      <>
        {this.state.guilds != null ? (
          <>
            <div className='config-variable'>
              <label>
                ...
                <FormBoolSwitch
                  name='autosendngrok-enabled'
                  checked={this.state.config.autosendngrok?.enabled}
                  onChange={this.handleDiscordChange}
                />
              </label>
            ...
            <div className='config-variable'>
              <label>
                ...
                <FormBoolSwitch
                  name='sharenotif'
                  checked={this.state.config.sharenotif}
                  onChange={this.handleDiscordChange}
                />
              </label>
            </div>
            <div className='config-variable'>
              <label>
                ...
                <FormBoolSwitch
                  name='crashreport'
                  checked={this.state.config.crashreport}
                  onChange={this.handleDiscordChange}
                />
              </label>
            </div>
          ...
          <div className='config-variable'>
            Discord isn't logged in. Input your bot token and invite the bot to your server to
            assign a channel to auto send Ngrok links.
          </div>
        )}
      </>
    );
    ...
    let discord = (
      <div className='config-discord'>
        {this.state.config != null
          ? ''
          : 'Create an app on Discord Developers to make a bot token. Then add your Discord user ID as the master.'}
        {loginFields}
        {this.state.config != null ? configFields : null}
        <div className='save-commands'>
          <button
            type='button'
            id='saveDiscordButton'
            className='save-button'
            onClick={this.saveDiscord}
          >
            Save
          </button>
          <div id='discordSaveStatusText' className='save-status'></div>
        </div>
      </div>
    );
