...
obs.on("InputVolumeMeters", (event)=>{
	console.log("VOLUME METER", event);
});
...
class Monitor extends React.Component{
	...
	obsRemoteConnect(){
		...
		obs.connect('ws://'+this.state.obs.ip+":"+this.state.obs.port, this.state.obs.password,{
		...
		.then(() => {
			...
			console.log("ERROR RESETTING STATE", err);
			this.setState(Object.assign(this.state, {obs:{
				ip:null,
				port:null,
				password:null
		...
		return(
			<div className="monitor-container">
				<div className="chat-channel-title">
					...
					<div className="chat-channel-remote">
						<button className="obs-connect-button" onClick={this.restartChat}>Restart Chat</button>
					</div>
				</div>
